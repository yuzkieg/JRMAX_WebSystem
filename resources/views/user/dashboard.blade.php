	...
	<div class="max-w-7xl mx-auto flex items-center justify-between px-6 h-20">
		...
		<div class="relative">
			<div class="relative group">
				...
				<div class="absolute right-0 mt-0 w-40 bg-black/80 text-white rounded-md shadow-lg p-3 z-50 backdrop-blur-sm opacity-0 pointer-events-none transform translate-y-2 scale-95 transition-all duration-200 ease-out group-hover:opacity-100 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:scale-100">
					<form method="POST" action="/logout">
						@csrf
						<button type="submit" class="cursor-pointer w-full text-left px-3 py-2 rounded hover:bg-red-600/30 transition">
                        <img src="{{ asset('assets/logout.png') }}" alt="Logout" class="inline w-6 h-6 mr-2" />
                        Logout</button>
					</form>
...
<section id="home" class="relative w-full min-h-[750px] lg:min-h-[910px] overflow-hidden">
	...
	<div class="absolute bottom-25 left-1/2 -translate-x-1/2 w-full max-w-[950px] ">
		<div class="bg-black/70 backdrop-blur-xl p-8 rounded-2xl shadow-2xl text-white mx-4 book-now-blur">
			<div class="grid grid-cols-1 md:grid-cols-5 gap-6">
				<div class="md:col-span-2">
					<div class="flex items-center space-x-2 mb-4">
						<img src="{{ asset('assets/calendar.png') }}" class="w-6 h-6" />
					...
					<div class="grid grid-cols-2 gap-4">
						<div>
							...
							<input type="date"
								class="w-full px-4 py-3 rounded-lg bg-white/10 border border-gray-500 text-white focus:outline-none focus:ring-2 focus:ring-red-500" />
						</div>
				...
				<div class="md:col-span-2">
					<div class="flex items-center space-x-2 mb-4">
						<img src="{{ asset('assets/fast-time.png') }}" class="w-6 h-6" />
				...
				<div class="flex items-end">
					<button class="booknowbtn w-full py-4 rounded-md bg-gradient-to-r from-red-700 to-red-500 drop-shadow-[0_6px_12px_rgba(255,255,255,0.45)] hover:drop-shadow-[0_10px_20px_rgba(255,255,255,0.65)] hover:scale-[1.03] active:scale-95 transition-all duration-300 cursor-pointer">Book Now</button>
				</div>
...
<section id="services" class="bg-[#1A1F24] text-white py-24 relative w-full min-h-[750px] lg:min-h-[910px]">
	...
	<div class="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 px-6">
		...
		<div class="service-card group relative rounded-2xl overflow-hidden shadow-xl cursor-pointer hover:scale-[1.03] transition-all duration-500">
			...
			<div class="absolute inset-0 flex flex-col items-center justify-center opacity-0 translate-y-5 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
