...
class FleetMaintenanceController extends Controller
{
    ...
    public function edit($id)
    {
        try {
            ...
            \Log::error('Fleet maintenance edit error: ' . $e->getMessage());
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Set current fleet user as reporter
            ...
            \Log::error('Fleet maintenance store error: ' . $e->getMessage());
            return response()->json([
                'success' => false, 
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            \Log::error('Fleet maintenance update error: ' . $e->getMessage());
            return response()->json([
                'success' => false, 
                'message' => 'Server error: ' . $e->getMessage()
            ], 500);
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            if (empty($user->password)) {
                \Log::warning('Fleet user has no password set for delete attempt: ' . $user->id);
                return response()->json([
                    'success' => false,
            ...
            // Check if password is correct (fleet user's own password)
            if (!Hash::check($password, $user->password)) {
                \Log::warning('Password mismatch for fleet maintenance delete by user: ' . ($user->email ?? $user->id));
                return response()->json([
                    'success' => false,
            ...
            \Log::warning('Fleet maintenance delete validation error: ' . json_encode($e->errors()));
            return response()->json([
                'success' => false,
                'message' => 'Validation error',
                'errors' => $e->errors()
            ...
            \Log::error('Fleet maintenance delete error: ' . $e->getMessage() . ' ' . $e->getFile() . ':' . $e->getLine());
            return response()->json([
                'success' => false,
                'message' => 'Server error: ' . $e->getMessage()
            ], 500);
    ...
    public function updateStatus(Request $request, $id)
    {
        try {
            ...
            \Log::error('Fleet update status error: ' . $e->getMessage());
